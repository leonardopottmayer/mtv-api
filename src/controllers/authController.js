...
module.exports = {
  register: async (req, res) => {
    ...
    if (!tel) {
      return res.status(422).json({ message: "Please insert tel number." });
    }

    const telExists = await User.findOne({ tel: tel });

    if (telExists) {
      return res.status(422).json({ message: "Please use another tel." });
    }
    ...
    try {
      ...
      res.status(500).json({
        message: "There was an error on the server, try again later.",
      });
    }
  },
